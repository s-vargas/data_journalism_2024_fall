...
Frederick <- Frederick |> 
  ...
  split = ifelse(str_detect(`Ballot Style`, "Rep") & REP_BOE != "REP" | str_detect(`Ballot Style`, "Dem") & REP_BOE == "REP" , 1 , 0)) # if they are REP and didnt vote for one of the 1776 ed candidates OR Dem and did vote for one of the 1776 candidates, they DID split their PRIMARY ballot
table(Frederick$split) 
# 34949 not split
# 15485 split from party candidates

Frederick |> 
  filter(str_detect(`Ballot Style`, "Rep")) |> 
  group_by(split, REP_BOE) |> 
  summarise(Rep_count = n())

Frederick |> 
  filter(str_detect(`Ballot Style`, "Dem")) |> 
  group_by(split, REP_BOE) |> 
  summarise(Dem_count = n())
...
Washington <- Washington |> 
  ...
  split = ifelse(str_detect(`Ballot Style`, "Rep") & REP_BOE != "REP" | 
                       str_detect(`Ballot Style`, "Dem") & REP_BOE == "REP" , 1 , 0)) # if they are REP and didnt vote for one of the 1776 ed candidates OR Dem and did vote for one of the 1776 candidates, they DID split their PRIMARY ballot
table(Washington$split) 
# 12965 not split
...
Talbot_BOE <- Talbot[!is.na(Talbot$`Board of Education (2) (50000173444)`), ] # only include those who had ballots with BOE on it
table(Talbot_BOE$split) 
...
AnneArundel <- AnneArundel |> 
  mutate(BOE = coalesce(`Board of Education (1) (50000176644)`, `Board of Education (3) (50000173844)`, `Board of Education (4) (50000176744)`, `Board of Education (5) (50000176844)`)) |> 
  mutate(REP_BOE = case_when(
    BOE == "Chuck Yocum (10024024.0000151)" ~ "REP",
    BOE == "LaToya Nkongolo (10008008.0000078)" ~ "REP",
    TRUE ~ "Other"
  ),
  split = ifelse(str_detect(`Ballot Style`, "Rep") & REP_BOE != "REP" | 
                       str_detect(`Ballot Style`, "Dem") & REP_BOE == "REP" , 1 , 0)) # if they are REP and didnt vote for one of the 1776 ed candidates OR Dem and did vote for one of the 1776 candidates, they DID split their PRIMARY ballot

AnneArundel_BOE <- AnneArundel[!is.na(AnneArundel$BOE), ] # only include those who had ballots with BOE on it
table(AnneArundel_BOE$split) 
...
###### lets make a DF of what we have observed through this analysis #######

County <- c("Anne Arundel","Frederick","Talbot","Washington")
Rep_Count <- c(19853, 20457, 573, 14039)
Rep_Split <- c(15950, 9127, 334, 7302)
Rep_Split_Perc <- round((Rep_Split/ (Rep_Split+Rep_Count) * 100), 2)
Dem_Count <- c(29631, 25772, 601, 8980)
Dem_Split <- c(2707, 2153, 82, 2752)
Dem_Split_Perc <- round((Dem_Split/ (Dem_Split+Dem_Count) * 100), 2)
Splitting <- data.frame(County, Rep_Count, Rep_Split, Rep_Split_Perc, Dem_Count, Dem_Split, Dem_Split_Perc)
print(Splitting)
```
