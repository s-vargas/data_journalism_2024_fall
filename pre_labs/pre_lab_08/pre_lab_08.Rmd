...
**Answer**
The first line is adding a new variable to the md_voters dataframe 
that calculates the difference of unaffiliated voters from 2022 to 2024 for each 
observation (county). I filled in the two variables for Unaffiliated voters from
the two years. 
The maryland_top_counties dataframe is using the data from the md_voters data
and only displaying the county and new unaffiliated difference variable. Then
the data is organized by descending order of unaffiliated difference from largest
to smallest growth.Finally, the dataframe only displays the top 10 from this list.
...
I see an empty chart, just gray space where a graph would be.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
There is a bar chart of counties on the x axis and UNAF_DIFF count on the y axis. 
This shows us side by side how the top 10 counties match up in their diff count
...
Now they are ordered from smallest to greatest UNAF_DIFF value. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
We have flipped the axes to make the names more visible by stacking them
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
We have changed the theme so that the chart appears a little cleaner, dark bars on light background. 
```{r}
maryland_top_counties |>
  ggplot() +
  geom_bar(aes(x=reorder(County,UNAF_DIFF), weight=UNAF_DIFF)) +
...
The entire background is now a light blue rather than white, and there are no
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
The axis labels make more sense, and the graph has a title now. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** 
I see a list of numbers of total contributions as grouped by dates. 
...
This is a line graph showing the daily fluctuations of donation counts over time. 
With months being the x axis and number of contributions on the y axis. 
```{r}
md_winred_by_date |>
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
Now we see the x axis broken up into more granular week by week dates. We 
are better able to distinguish specific dates or approximate dates for spikes. 
```{r}
md_winred_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
The dates are not repositioned so they are easier to read at an angle instead of
placed next to each other where they overlap. 
```{r}
md_winred_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
