...
**Answer**
For the most part, yes they have the same column structure with the order of "UNA" 
and "OTH" flipped around.
```{r}
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows, and there should be 24 rows
...
The county with the highest 18+ black population is PG county, which also has
the highest density of Democrats of all the counties. 
...
Howard county, home of Ellicott City and 'Korean Way', has the highest proportion
of Asian voters. This same county however is majority Democrat, but not by a lot. 
While PG county is known for being densely black and Democratic, Howard county is less 
homogenous in race and partisanship.
```{r}
maryland_population_with_voters_and_demographics |>
