...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
**Answer**
They mostly seem to follow the same themes, but if there is anything different
I would point out how the first 10 has more names that are "inspired by...", with 
one duplicate, a Shakespeare name, and Loki (which was not explicitly stated as inspired by).
In the first 10 there is also only one name that gives no description outside of "for
a female lemur", and three in the last 10. 
```{r}
...
**Answer**
It got it pretty right. I would also add onto the prompt a column for information about
pay/compensation, as well as who to contact for application submission, application deadlines, and 
...
**Answer**
The response would make sense before the |>  pipe operator became a part of the 
base r functions in the new 4.1 version of R. Maybe this response was true before, 
but this is not a fact machine and may still require me to seek more help. 
```{r}
...
**Answer**
The output makes sense. The things it suggests includes: distributions of registration dates and birthdays,
a graph of party affiliation, a graph of states' frequencies in addresses, voter ages, and
voter registration numbers by month. This would all help me understand the data to an extent. 
This makes assumptions about the data including registrations for multiple states. Usually
registration data is more specific to individual states or even more specific.
...
**Answer**
The part where it describes the sunglasses is a little wonky, and I think it's interesting that the model chose to take a guess at what is in the man's bag. I would change those descriptions, with a better estimate of the glasses and helmet, and no guessing about the bag as it is not visible and unecessary. It was unable to recognize or comment on the hand in the forefront seemingly belonging to the person who took the photo. 

The image features a member of the US military who is an elite free-fall operations specialist dropping from an aircraft.  He is wearing a tan and green military uniform, consisting of an ACU (Army Combat Uniform) top and a green helmet and gloves, while sporting black sunglasses with reflective black and camouflaged visor. A green-and-tan camouflaged garrison cap helmet is strapped to the helmet containing the sunglasses. A backpack is strapped to the specialized uniform, likely containing specialized military equipment, including a spare chute, oxygen mask, altimeter and emergency radio. The scene was taken from a very high altitude, with only desert and hazy blue skies visible. Below the military member is a large black parachute attached to a box decorated with the US flag. 
The image was produced by A. Robinson Photography LLC in Las Vegas, Nevada, and has a watermark that prohibits any unauthorized use.
...
**Answer** 
- It wont run! And my desktop machine at home (that I'm running this on) is a nice new 32GB ram and A770 graphics card that I built this year.
- ACTUALLY after running it again, closing other programs, and waiting 9 minutes it finally ran
The results dont really make sense, the columns list things like:
KamalaHarris.com	
Team Trump	
Elon Musk + Trump Alert	
Letter to Trump	
Trump Merch Team	
Photo from Trump	- this one makes sense
Trump Defenders (via Team Marsha)
Donald J. Trump	
Kamala Harris	
Trump Supporter Alerts
Trump Award Committee	
Trump Inauguration Team
Trump Victory AR15

I am unsure what it picked up to parse these specifics out, but
the instructions seem to not be specific enough for the model to extract what was intended. 
```{r}
emails |>
...
+   llm_extract(body, "merchandise that can be won")
Error in `mutate()`:
ℹ In argument: `.extract = llm_vec_extract(x = body, labels = labels, additional_prompt =
  additional_prompt)`.
Caused by error in `httr2::req_perform()`:
! Failed to perform HTTP request.
Caused by error in `curl::curl_fetch_memory()`:
! Operation was aborted by an application callback [127.0.0.1]
Backtrace:
  1. mall::llm_extract(emails, body, "merchandise that can be won")
 10. mall::llm_vec_extract(x = body, labels = labels, additional_prompt = additional_prompt)
 11. mall:::m_vec_prompt(...)
 13. mall:::m_backend_submit.mall_ollama(...)
 14. mall (local) map_here(...)
     ...
