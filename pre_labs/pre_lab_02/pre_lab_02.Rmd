...
```{r}
head(primary_24) # this gives me the top 6 rows of the data
summary(primary_24) # this gives me summary for each variable in the dataset (length, type of var)
colnames(primary_24) # this gives me a list of the column names (variables)
glimpse(primary_24) # this shows me the first few observations in each column 
```
...
* selecting what specific columns I want this table to present
* creating a new variable through an equation of two other variables, 
specifically calculating how many votes cast were cast on election day for 
...
primary_24 |>
  select(office_name, office_district, candidate_name, party, county_name, election_day, votes) |>
  mutate(
  percent_election_day = (election_day/votes)*100 # make it a % instead of "0.00"
)  |> 
  arrange(desc(percent_election_day)) # arrange by biggest election day turnout first
```

How did the results change from the previous task, and why?

**Answer**
* This time the table that is presented is organized by percent election day 
turnout for each candidate in descending order. 
...
**Answer**
* I think this is just as interesting as the last table. The difference in the party
breakdown of election day turnout is interesting. At the most turnout on election day, it is mostly Republican candidates, but there are some Democrats as well. At the candidates with the least votes on election day, it is all Democrats. Alternate forms of voting (VBM, Mail-in, early) are politicized, especially in recent decades, and this data reflects that.
...
* str_to_title() converts only the first letter of each word to capital

* str_to_sentence() converts only the first letter of sentence to capital
...
**Answer**
* The mutate statement is creating a new variable with the following parameters:
** When the variable in maryland_winred_contributions amount is < 100, the new var amount_category is going to observe "Less than 100" 
** when amount is >= 100 & amount < 500, amount_category = "Between $100 and $499"
** amount is >= 500 & amount < 1500, amount_category = "Between $500 and $1499"
** amount is >= 1500 & amount < 3000, amount_category = "Between $500 and $2999"
** amount is >= 3000, amount_category = "$3,000 or more"
...
**Answer**
That allows for the new table to be observable in the viewer, it is now its own item that shows up in the global environment.
...
**Answer:**
* the expenditure date has a new format yyyy-mm-dd
...
**Answer**
* The new month column takes the expenditure date data, simplifies and rounds the dates to the month through floor_date(), and then groups the entire data by month. This helps to see total donations in specific months. We can see when people start paying attention, opting into the election season, and particpationg when things ramp up. 