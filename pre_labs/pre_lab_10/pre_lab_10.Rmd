...
10 columns consisting of precinct identifiers, district numbers, and numerical shape characteristics. 
```{r}
...
Precinct information of individual voting locations, their districts, and  geographic shape data.
```{r}
View(pg_precincts)
```
...
It is a map of PG county, with shaded in precinct areas. There are latitude and longitude axes.
```{r}
pg_precincts |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Tigris was libraried successfully
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The dataset includes 3,235 counties in the US in 2022. There are county identifiers
such as name, zip, state, and other values. There are also those shape file variables
...
**Task** Run the following code. Describe the output in the space below. **Answer**
24 observations in a dataset that filtered from the counties data for counties only in MD. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
A map of MD with all 24 counties, but the Eastern Shore is encompassing the Chesapeake. 
```{r}
md_counties |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This is a map of PG county with all of the precincts outlined and shaded in with
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The code produces a dataset of turnout by precinct and voting method. There is also
a column of the number of total eligible voters in the precinct.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Now we see a dataset that has merged the shapefiles data with the precinct turnout data. 
We can use this to map turnout percentages. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Its a map of PG county with precincts outlined and shaded according to eligibility. 
```{r}
ggplot() +
...
It looks like theres few eligible voters in most precincts. I wonder how much of this
comes from population density. The darkest precincts seem to be on the farther edges of the county, 
but they are also larger precincts, which is interesting. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**
A table of counties, registered eligible voters by party affiliation, and total, sorted by total. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This is a table of ACS data from MD showing the population estimates of each county. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This table includes the shapefiles as well as the county population estimates. 
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              state = "MD",
...
**Task** Run the following code. Describe the output in the space below. **Answer**
These are our two datasets we are going to join, one with the shapefiles and population estimates, 
and the other with eligible voters by party and county. 
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The shapefile data has been cleaned up with the county names matching the format of 
our county names in the other data. The titles of the County and Population variables were renamed,
and the only columns we need were kept. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
We have the shapefile data merged with the eligible voters by party data. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The counties that didnt match have been fixed so the proper data can be matched up
```{r}
md_county_population <- md_county_population |>
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The code added a new column that estimates the amount of eligible voters per 10000 people
in the estimated population per county
```{r}
md_voters_per_10k <- md_county_population |>
  left_join(voters_by_county) |>
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This is a map of MD with counties shaded by the proportion of voters per 10k residents.
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Now the map has the magma theme, which makes the differences in the voter density easier to discern.
```{r}
ggplot() +
  geom_sf(data=md_voters_per_10k, aes(fill=voters_per_10k)) +
  theme_minimal() +
...
There are higher voter densities on the Eastern Shore, and Garrett County. The more
rural parts have higher voters per 10k, while the metropolitan such as the DC metro
and Baltimore city/county areas have a lower rate of voters per 10k.  
```{r}
ggplot() +
  geom_sf(data=md_voters_per_10k, aes(fill=voters_per_10k)) +
  theme_minimal() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This code creates a column of the amount of Unaffiliated voters per 10000 people
```{r}
...
This map shows us the proportion of specifically UNAF voters per 10k in each county. In this map 
it looks like Howard, Anne Arundel, and Frederick counties have the highest proportions
of UNAf voters, versus the rest of the state being about in the middle. Although
places like Baltimore city, garrett county, pg county, and a county on the eastern
shore have very low proportions of UNAF voters, likely due to strong partisanship. 
```{r}
ggplot() +
