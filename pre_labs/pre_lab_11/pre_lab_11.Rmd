...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** The columns copy over exactly as shown on the site. The numbers include commas, and the column titles are the same long titles.
...
The numbers are referring to each of the columns in their respective place to avoid copying the long names and whatever formatting comes with it. 
```{r}
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **Answer** There are 55 <li> tags
...
The output shows the text inside each of the <li> elements. 
They look like committee names and other variables/column names mixed in. 
```{r}
...
We have a dataset of 55 committees with the respective / urls to put after house.gov. 
