...
The data read in one of the rows as the headings. There are many NAs from the blank cells
on the PDF.
...
We can't tell what columns are what with the default titles. We also can't disaggregate party as easily.
```{r}
...
There are descriptive column names now for the first 7 columns, A new column 'Office' was created to 
indicate House vs Senate campaigns.There are still a lot of unnamed columns
```{r}
benzie_2024 <- benzie_2024 |>
