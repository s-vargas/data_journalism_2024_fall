...
[Graphic](https://datawrapper.dwcdn.net/61bcC/1/)
...
md_gov_county <- md_gov_county |> 
  mutate(tot = Cox + Moore + Lashar + Wallace + Harding + `Write-ins`,
         pct_Moore = (Moore/tot)*100)
...
The previous map showed the entire US, with the counties 
not observed in our data being grayed out. This new map
shows just Maryland and no gray unused space. Just Maryland counties. 
...
Now instead of unlabled meaningless values to a viewer,
the counties are labeled by name and the pct_Moore vote is 
readable. 
...
[Graphic](https://datawrapper.dwcdn.net/70YGf/1/)
