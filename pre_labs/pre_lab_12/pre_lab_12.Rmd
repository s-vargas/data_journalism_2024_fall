...
999 observations of 4 variables. Most recent 999 releases with date, url, title, and text. 
```{r}
...
Unique shows us each word that is used, and lists them once each. 
```{r}
...
The output is a 1 variable dataset of 677,425 words used across the texts. 
```{r}
...
This isn't interesting! A lot of parts of speech that are simple articles of a sentence. 
...
Definitely more interesting than before, we see words that conern people and policy more now. 
```{r}
unique_words |>
...
Things like health and baltimore were flipped, and health was a lot less discussed. 
This would make sense due to something like the destruction of the Key bridge and 
subsequent federal/state support that were needed.
```{r}
unique_words_2022 |>
  anti_join(stop_words) |>
  group_by(word) |>
...
**Task** Run the following code and describe the results. **Answer**
We see some policy related terms and community/geography related terms. This shows
us what certain focuses may come up a lot. We see health care, Prince George's, 
location Baltimore, covid 19, human rights. 
```{r}
releases |>
  filter(year(date) == 2022) |>
...
**Task** Run the following code and describe the results. **Answer**
This looks similar to the list above, with covid gone.
The list is primarily terms around place, money, and policy. 
```{r}
releases |>
  filter(year(date) == 2024) |>
  unnest_tokens(bigram, text, token = "ngrams", n = 2) |>
...
For the press release data we may prefer the lemmatized words, since its able to 
create a smaller dictionary of unique words than stem is. This helps to get to the true 
content words that we want to analyze. 
```{r}
...
The lemmatized data is more useful, as it was successfully able to group words together
such as "funding" and "funds". This lets us group that topic together and more 
accurately represent the data in the table. 
```{r}
...
This is interesting, as the amount of times that support was mentioned was less in 2024
than in 2022. The number one spot still has a significantly larger n than the second spot.
The sentiment analysis seems pretty accurate. 
```{r}
