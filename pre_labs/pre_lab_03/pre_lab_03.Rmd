...
There are 1640 rows/errors. The problem seems to be that the data is not formatted consistently. The library guessed that a given column was a “logical” data type. When it hit a value that did not look like a True or False value, it didn’t know what to do, and didn’t read the value correctly.
```{r}
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** district is a double numerical value, precinct is a character, and mail is also double.
```{r}
...
402345 NA mail results. This shows that counties either didn’t report votes by mail or they had a different name/category for it.
```{r}
...
The first 20 results suggest that there is either a placeholder date, or this voter file isn't thoroughly maintained, as 120 year old voter is highly unlikely. 
```{r}
yadkin_voters |> 
...
The amount of deceased voters still in the file that have not been purged is concerning. I've done some previous research with NC registration data (because of the accessbility), and was under the impression NC was strict about voter roll maintenance and purges. I would also really enjoy looking into the felony conviction/sentence completed statuses. I've also done work on felony convictions and voter registrations in Florida in 2020 after the passage of Amendment 4, super interesting stuff. 
```{r}
...
character, when it should probably be double
...
**Answer**
It seems that this data has a lot of formatting inconsistencies- very dirty data. Conowingo is mispelled or varied in the city variable, Merry Knoll Lane/Ln has multiple variations, and the zip codes are input as 5 digit or 9 digit codes. All of these issues make it hard to group data and accurately analyze it if two things that mean the same thing are not going to appear together when I need it to.
...
**Answer**
It has added an x to the header name so that it does not start with a number. Helps to standardize the headings and create some consistency when calling columns
...
This looks like you cannot use sum() on a character value, because you cannot add up strings.
...
Derrick Hamilton is the only duplicate, with one donation appearing twice in the data. 
```{r}
cleaned_conowingo |> 
  get_dupes()
```
...
This standardized all of the zip codes by only maintaining the first 5 character objects in each string. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 
...
There is 2 mispellings left after this round of cleaning, Conowing and Conowingoo. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 
...
1 misspelling remains, Conowingoo. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 
...
There are no misspellings left. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 
